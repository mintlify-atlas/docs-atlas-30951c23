...
description: "API reference for RolePermission, GroupPermission, and UserPermission dataclasses plus resolve_permission and resolve_editable_fields helpers."
---
...
The function iterates `permissions` and includes any entry that is an instance of `RolePermission` or `UserPermission` (checked via `isinstance(p, (RolePermission, UserPermission))`). Each included entry is rendered using `p.role` — this works for `RolePermission` and `GroupPermission` instances which have a `.role` attribute.
