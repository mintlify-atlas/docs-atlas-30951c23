---
title: "POST /var-cms/api/media/convert/ — Convert Media File Formats"
sidebarTitle: "Media Convert"
description: "POST /var-cms/api/media/convert/ converts images, audio, and video files to different formats. Images use Pillow; audio and video require ffmpeg; PDF to images requires pdf2image."
---
...
<ParamField body="target_fmt" type="string" required>
  Desired output file extension (without the leading dot), lower-cased. The endpoint uses this to determine both the conversion pathway and the output filename.
...
<ResponseField name="error" type="string">
  ...
  | ------ | ------------- | -------------- | ----- |
  | `400` | `"file_path and target_fmt required"` | — | One or both required POST parameters were missing. |
...
Pillow is a **required** core dependency and must always be present. Audio/video conversion and PDF extraction are opt-in:

```bash
# Core: install Pillow (always required)
pip install django-var-cms pillow

# PDF page extraction (adds pdf2image):
...
### Images
Pillow opens the source file, converts to the target format, and saves at quality `90`. RGBA images are automatically converted to RGB when the target is JPEG.

### Audio & Video
ffmpeg is invoked via `subprocess` with a simple `-i <source> <output>` command and a 120-second timeout. The converted file is written to a temporary path, read into memory, then saved via Django's `default_storage`.
