---
title: "Registering Django Models with django-var-cms"
sidebarTitle: "Registering Models"
description: "Learn how to create var_cms_admin.py files, define VarCMSModelAdmin subclasses, and register Django models with the global var_cms_site registry."
---
...
Every attribute on `VarCMSModelAdmin` is optional — sensible defaults are applied automatically. The table below covers all basic configuration options.
...
Called whenever an object is saved (both create and update). The default implementation auto-populates `slug` from `title`/`name`/`headline` if the field is empty, then calls `obj.save()`.
